...
class DiscoverMoviePagingDataSource(
    ...
    override fun loadInitial(
        ...
        coroutineScope.launch(Dispatchers.IO) {
            ...
            try {
                val response = tmdbApiService.getDiscoverMovie("en-US", "popularity.desc", 1)
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, FilmGist>) {
        coroutineScope.launch {
            networkState.postValue(NetworkState.LOADING)
            try {
                val response = tmdbApiService.getDiscoverMovie("en-US", "popularity.desc", params.key)
                val totalPage = response.totalPages
